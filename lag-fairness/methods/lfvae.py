...
class LagrangianFairTransferableAutoEncoder(VariationalAutoEncoder):
    ...
    def test(self):
        ...
        print(f"Saving embeddings to {self.logdir}/embedding.npy")
        embeddings = {"train":self.get_train_embeddings(), "test": self.get_test_embeddings()}
        np.save(f"{self.logdir}/embedding.npy", embeddings, allow_pickle=True)
    ...
    def get_train_embeddings(self):
        ...
        while True:
            try:
                ks = self.sess.run([self.x, self.y, self.u, self.z])
                # import ipdb; ipdb.set_trace()
                x.append(ks[0])
                y.append(ks[1])
                u.append(ks[2])
                ...
                break
        return {"x": np.concatenate(x, axis=0), "y": np.concatenate(y, axis=0), "u": np.concatenate(u, axis=0), "z": np.concatenate(z, axis=0)}
