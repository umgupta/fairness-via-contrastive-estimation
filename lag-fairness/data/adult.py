...
def save_adult_datasets():
    adult_data = pd.read_csv('adult.data.txt', header=None, sep=', ').to_numpy()
    adult_test = pd.read_csv('adult.test.txt', header=None, sep=', ').to_numpy()
    ...
    def gather_labels(df):
        ...
        for j in range(df.shape[1]):
            if type(df[0, j]) is str:
                ...
                labels.append(np.max(df[:, j]))
        return labels

    adult_data = remove_question(adult_data)
    adult_test = remove_dot(remove_question(adult_test))
    adult_labels = gather_labels(np.concatenate([adult_data, adult_test],axis=0))
    ...
    def transform_to_binary(df, labels):
        ...
        for j in range(len(labels)):
            if type(labels[j]) is list:
                ...
                idx += 1
        return d.astype(np.float), u.astype(np.int), y.astype(np.int)  # observation, protected, label
    ...
    return adult_binary, adult_test_binary
