...
class VariationalEncoder(object):
    def __init__(self, z_dim=8):
        ...
        def discriminate_func(z):
            fc1 = tf.layers.dense(z, 100, activation=tf.nn.relu)
    ...
    def sample_and_log_prob(self, x, u, y):
        ...
        def log_prob(v, a):
            v = tf.cast(v, tf.float32)
            return tf.reduce_mean(
                -tf.nn.softmax_cross_entropy_with_logits(logits=a, labels=v))
...
class VariationalDecoder(object):
    def __init__(self, z_dim=8, x_dim=71):
        self.z_dim = z_dim
...
def main(mi, e1, e2, e3, e4, e5, disc, lag, test, gpu):
    ...
    if gpu == -1:
        ...
        device_id = tu.find_available_gpu()
    else:
        device_id = gpu
    ...
    if lag:
        ...
        logdir = tu.obtain_log_path('fair/mifr_n/test-health/{}-{}-{}-{}-{}-{}-{}/'.format(mi, e1, e2, e3, e4, e5, disc))
